# Extend to 2 other PDA scenarios with different forcing intensities
...
intensity_setting_levels <- c(1, 3)
rep <- seq_len(n_sim)
settings_dt <- data.table(expand.grid(rep = rep, intensity_setting_level = intensity_setting_levels))
...
for(i in seq_len(nrow(settings_dt))) { 
  ...
  dataset_i <- generate_SHM_dataset(n = 500, 
                                    ...
                                    sigma = 0.25,
                                    mu_init = c(0, 0), 
                                    intensity = settings_dt[i, intensity_setting_level],
