...
# in this setting, 28 of the 150 replciates failed!
# these were all at at mu = 2 (the most 'stiff' setting)
# so actually it was 28 out of 50.
# It seems that for the stiffer setting, the non discretised 
# option needs to be ran (which makes sense, because discretisation is
# of the STM).
...
# [1] 102 106 107 108 109 111 112 114 115 120 121 123 127 128 130 131 133 134 136 137 141 142 143
# [24] 144 145 146 147 148
...
for(i in null_result_inds) {
  ...
  if(class(try_not_optimised_code) != "try-error") {
    ...
    print("Failed again with non-optimised code")
  }
