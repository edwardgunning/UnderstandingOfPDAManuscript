...
for(i in seq_len(num_simulations)) {
  ...
  dataset_i <- generate_SHM_dataset_general(n = n_i,
  ...
  D2x_i_test <- - x_i_test + eps_i_test
...
matlines(beta_array_simulations[,1,10,], col = 4)
lines(apply(beta_array_simulations[,1,10,], 1, mean))
abline(h = 0)
...
matlines(beta_array_simulations[,2,10,], col = 4)
lines(apply(beta_array_simulations[,2,10,], 1, mean))
...
matlines(beta_array_simulations[,3,10,], col = 4)
lines(apply(beta_array_simulations[,3,10,], 1, mean))
