...
dat = generate_SHM_dataset(n = 10000,
...
Sig = (0.5 ^ 2) * dnorm(x = 2 * outer(tfine,tfine,'-'))
...
Sig2 <- eval.bifd(sevalarg = tfine, tfine, var.fd(pdaest$yfdobj - pdaest$yhatfdobj))
...
par(mar=c(5,5,1,1))
matplot(tfine,betafine,type='l',ylim=c(-2,0.5),lwd=2,lty=1,col=c('darkblue','darkred'),
