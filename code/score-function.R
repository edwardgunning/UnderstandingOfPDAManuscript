...
Ord2ODEdc = function(t,x,betafd){
	...
	J = matrix(c(0,1,betavals[1],betavals[2]),2,2)
...
dat = generate_SHM_dataset(n = 5000,
...
tfine = grid_points									
bbasis = create.bspline.basis(range(tfine),nbasis = 51)

betaC = cbind(rep(-1,bbasis$nbasis),rep(0,bbasis$nbasis))
betafd = fd(betaC,bbasis)

Xfd = Data2fd(tfine,dat$x,bbasis)

Sig = (0.25 ^ 2) * dnorm(x = 2 * outer(tfine,tfine,'-'))
...
betabasis = create.bspline.basis(range(tfine),nbasis=5)
...
lines(fd(betaest.s,betabasis),lwd=2)
abline(h=-1)
...
#	txest = xest
#	txest[i] = txest[i] + 1e-1
#	
#	Gest[,i] = ScoreFn(txest,Xfd,betabasis,tfine,Sig)/1e-1
#}

#iGest = solve(Gest)

#Sest = ScoreFndc(hmm.small$x,Xfd,betabasis,tfine,Sig)
...
betafine = bvals%*%matrix(betaest.s, 5, 2)
...
betavar = cbind(bvals,bvals)%*%iGest%*%scov%*%t(iGest)%*%t(cbind(bvals,bvals))*nfine/diff(betabasis$range)

betaest.sd = matrix(sqrt(diag(betavar)),nfine,2,byrow=FALSE)

par(mar=c(5,5,1,1))
matplot(tfine,betafine,type='l',ylim=c(-1.5,0.25),lwd=2,lty=1,col=c('darkblue','darkred'),
