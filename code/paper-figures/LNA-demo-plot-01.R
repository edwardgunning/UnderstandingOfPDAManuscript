...
# Import results: ---------------------------------------------------------
results_list <- readRDS(file = here::here("outputs", "VDP", "LNA-demo-01.rds"))
...
plot_dt[, sigma_forcing := factor(sigma_forcing, 
                                levels = c(0.1, 0.2, 0.5),
                                labels = paste0("$\\sigma = ", c(0.1, 0.2, 0.5), "$"))]
...
p <- ggplot(data = plot_dt) +
  ...
  facet_wrap(var ~ sigma_forcing, nrow = 2, ncol = 3, scales = "free_y") +
  geom_line() +
...
tikz(here::here("outputs", "VDP", "paper-plots", "LNA-demo-01.tex"),
