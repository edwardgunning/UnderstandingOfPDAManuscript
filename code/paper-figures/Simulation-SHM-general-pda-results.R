...
source(here::here("SHM-general-data-generation.R"))
...
results_path <- here::here("outputs", "SHM", "simulation-results", "general-SHM-simulation.rds")
...
dataset_1 <- generate_SHM_dataset_general(n = n,
...
tikz(here::here("outputs", "SHM", "paper-plots", "general-shm-datasets.tex"),
...
hline_dataset <- data.table(parameter = c("$\\alpha (t)$", "$\\beta_0 (t)$", "$\\beta_1 (t)$"),
                            values = c(0, -1, 0))
hline_dataset[,
              parameter := factor(parameter, 
...
results_plot <- ggplot(data = plot_params_dt_lng) +
  aes(x = t, y = value, group = interaction(rep, estimator), colour = estimator) +
  facet_wrap(~ parameter, scales = "free_y", ncol = 3) +
  geom_hline(aes(yintercept=values), data = hline_dataset, lty = 2) +
  geom_line(alpha = 0.15) +
  geom_line(data = plot_params_dt_lng_average, aes(group = estimator), lwd = 1.1, lty = 1) +
...
tikz(here::here("outputs", "SHM", "paper-plots", "general-shm-result.tex"),
...
tinytex::lualatex(here::here("outputs", "SHM", "paper-plots", "general-shm-result.tex"))
