...
# Sigma = 0.1: ------------------------------------------------------------
inds_sigma_0.1 <- which(settings$sigma_forcing == 0.1)
results_sigma_0.1 <- results_list[inds_sigma_0.1]
estimates_sigma_0.1 <- map_dfr(.x = results_sigma_0.1, function(x) {
...
inds_sigma_0.2 <- which(settings$sigma_forcing == 0.2)
results_sigma_0.2 <- results_list[inds_sigma_0.2]
estimates_sigma_0.2 <- map_dfr(.x = results_sigma_0.2, function(x) {
...
estimates_sigma_0.1$sigma <- 0.1
estimates_sigma_0.2$sigma <- 0.2
estimates_sigma_0.4$sigma <- 0.4

estimates_dt <- data.table(rbind(estimates_sigma_0.1,
                   estimates_sigma_0.2,
                   estimates_sigma_0.4))
estimates_dt_lng <- melt.data.table(estimates_dt,
                                    id.vars = c("t", "sigma", "estimator", "sim_rep"),
                                    variable.name = "parameter",
                                    value.name = "value",
                                    variable.factor = FALSE,
                                    value.factor = FALSE,
...
estimates_dt_lng[, sigma := factor(sigma,
...
p <- ggplot(estimates_dt_lng) +
  aes(x = t, y = value) +
  facet_grid(parameter~sigma, scales = "free_y") +
...
tikz(here::here("outputs", "VDP", "paper-plots", "vary-sigma-results.tex"),
...
tinytex::lualatex(here::here("outputs", "VDP", "paper-plots", "vary-sigma-results.tex"))
